...
public class WeekView extends View {
    ...
    private int itemPerHour = 4;
    private int minute = itemPerHour * 24;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * minute + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * minute + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
                    break;
            }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / minute));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * minute - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * minute - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < minute; hourNumber++) {
    ...
    private Calendar getTimeFromPoint(float x, float y){
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0 && x > start && x < startPixel + mWidthPerDay){
                ...
                int hour = (int)(pixelsFromZero / (mHourHeight * itemPerHour));
                int minute = (int) (60 * (pixelsFromZero - hour * mHourHeight * itemPerHour) / (mHourHeight * itemPerHour));
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()){
                    ...
                    // Calculate top.
                    float top = mHourHeight * minute * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * minute * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretTime(int hour) {
                    ...
                    calendar.set(Calendar.HOUR_OF_DAY, hour / 4);
                    calendar.set(Calendar.MINUTE, (hour % 4) * 15);
    ...
    public void goToHour(double hour){
        ...
        if (hour > 24)
            verticalOffset = mHourHeight * minute;
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * minute - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * minute - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
